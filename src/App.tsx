...
const RetirementCalculator = () => {
  const [initialNetWorth, setInitialNetWorth] = useLocalStorage<number>(
    'initialNetWorth',
    10000
  );
  const [monthlyContribution, setMonthlyContribution] = useLocalStorage<number>(
    'monthlyContribution',
    500
  );
  const [years, setYears] = useLocalStorage<number>('years', 40);
  const [currentAge, setCurrentAge] = useLocalStorage<number>('currentAge', 30);
  const [stockAllocation, setStockAllocation] = useLocalStorage<number>(
    'stockAllocation',
    50
  );
  const [reitAllocation, setReitAllocation] = useLocalStorage<number>(
    'reitAllocation',
    10
  );
  const [cryptoAllocation, setCryptoAllocation] = useLocalStorage<number>(
    'cryptoAllocation',
    5
  );
  const [bondAllocation, setBondAllocation] = useLocalStorage<number>(
    'bondAllocation',
    25
  );
  const [realEstateAllocation, setRealEstateAllocation] =
    useLocalStorage<number>('realEstateAllocation', 10);
  ...
  const [realEstateCAGR, setRealEstateCAGR] = useLocalStorage<number>(
    'realEstateCAGR',
    6
  );
  const [annualInflationRate, setAnnualInflationRate] = useLocalStorage<number>(
    'annualInflationRate',
    3
  );
  const [initialDate, setInitialDate] = useLocalStorage<Date>(
    'initialDate',
    new Date()
  );
  const [currentDate, setCurrentDate] = useLocalStorage<Date>(
    'currentDate',
    new Date()
  );
  const [currentNetWorth, setCurrentNetWorth] = useLocalStorage<number>(
    'currentNetWorth',
    10000
  );
  const [annualExpenses, setAnnualExpenses] = useLocalStorage<number>(
    'annualExpenses',
    40000
  );
  const [withdrawalRate, setWithdrawalRate] = useLocalStorage<number>(
    'withdrawalRate',
    4
  );
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 bg-gray-800 text-gray-200 shadow-lg rounded-lg">
      ...
      <Tabs variant="boxed" onChange={(index) => setActiveTab(index as any)}>
        <Tabs.Tab
      ...
      <div className="mt-4">
        <div className="w-full max-w-8xl mx-auto p-4 shadow-lg mb-8">
          <h3 className="text-lg font-bold mb-2">
            Net Worth and Asset Growth Over Time
          </h3>
          ...
          {!fireDate && (
            <div className="mb-4 p-4 bg-red-600 text-white border border-red-800 rounded">
              You won't retire before {years} years. Try increasing your
              timeline.
            </div>
          )}
